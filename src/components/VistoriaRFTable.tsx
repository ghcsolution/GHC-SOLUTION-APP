  ...
  if (items.length === 0) {
    return (
      <div className="bg-white dark:bg-gray-900 rounded-3xl border border-gray-200 dark:border-gray-800 p-12 text-center">
        ...
        <h3 className="text-lg font-bold text-gray-900 dark:text-white">Nenhuma vistoria encontrada</h3>
        <p className="text-gray-500 dark:text-gray-400">Comece registrando uma nova vistoria RF.</p>
      </div>
    );
  }
    ...
    <div className="space-y-4">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50/50 dark:bg-gray-800/50 border-b border-gray-100 dark:border-gray-800">
                <th className="px-6 py-4 w-12">
                  <input 
                    type="checkbox" 
                    className="w-4 h-4 rounded border-gray-300 dark:border-gray-700 text-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-400 dark:bg-gray-800"
                    checked={allSelected}
                    ref={(el) => {
                ...
                <th className="px-6 py-4 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider">Site / Data</th>
                <th className="px-6 py-4 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider">Localização</th>
                <th className="px-6 py-4 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider">Foto 01 - Fachada</th>
                <th className="px-6 py-4 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider">Foto 02 - Placa</th>
            ...
            <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
              {items.map((item) => (
                <tr key={item.id} className={`group hover:bg-indigo-50/30 dark:hover:bg-indigo-900/10 transition-colors ${selectedIds.includes(item.id!) ? 'bg-indigo-50/20 dark:bg-indigo-900/20' : ''}`}>
                  ...
                  <td className="px-6 py-6">
                    <div className="flex flex-col">
                      <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{item.municipio || '-'}</span>
                      <span className="text-xs text-gray-400 dark:text-gray-500 font-bold">{item.uf || '-'}</span>
                    </div>
                  </td>
                  
                  <td className="px-6 py-6">
                    {item.foto_fachada ? (
                      <div className="relative w-24 h-16 rounded-xl overflow-hidden border border-gray-100 dark:border-gray-800 shadow-sm group/img">
                      ...
                      <div className="w-24 h-16 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-dashed border-gray-200 dark:border-gray-700 flex items-center justify-center">
      ...
      <div className="md:hidden space-y-4">
        {items.length > 0 && (
          <div className="flex items-center justify-between px-2 pb-2">
            <button 
              ...
              className="text-xs font-bold text-indigo-600 dark:text-indigo-400 flex items-center gap-2"
            >
              <input 
                type="checkbox" 
                readOnly
                className="w-4 h-4 rounded border-gray-300 dark:border-gray-700 text-indigo-600 dark:bg-gray-800"
                checked={allSelected}
                ref={(el) => {
                  if (el) el.indeterminate = someSelected;
                }}
        ...
        {items.map((item) => (
          <div 
            key={item.id} 
            className={`bg-white dark:bg-gray-900 p-4 rounded-2xl border transition-all ${selectedIds.includes(item.id!) ? 'border-indigo-500 ring-1 ring-indigo-500' : 'border-gray-200 dark:border-gray-800'} shadow-sm space-y-4`}
          >
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                <input 
                  type="checkbox" 
                  className="w-5 h-5 rounded border-gray-300 dark:border-gray-700 text-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-400 dark:bg-gray-800"
                  checked={selectedIds.includes(item.id!)}
                  onChange={() => onSelect(item.id!)}
                />
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider">Fachada</p>
                {item.foto_fachada ? (
                  <img src={item.foto_fachada} className="w-full aspect-video object-cover rounded-lg border border-gray-100 dark:border-gray-800" />
                ) : (
                  <div className="w-full aspect-video bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-dashed border-gray-200 dark:border-gray-700 flex items-center justify-center">
                    <Camera className="w-4 h-4 text-gray-300 dark:text-gray-600" />
                  </div>
                )}
              </div>
              <div className="space-y-1">
                <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider">Placa</p>
                {item.foto_placa ? (
                  <img src={item.foto_placa} className="w-full aspect-video object-cover rounded-lg border border-gray-100 dark:border-gray-800" />
                ) : (
                  <div className="w-full aspect-video bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-dashed border-gray-200 dark:border-gray-700 flex items-center justify-center">
                    <Camera className="w-4 h-4 text-gray-300 dark:text-gray-600" />
                  </div>
            ...
            <div className="flex items-center justify-between pt-2 border-t border-gray-50 dark:border-gray-800">
              <div className="flex items-center gap-1 text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase">
